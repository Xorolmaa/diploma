    ...
    const [post, setPost] = React.useState(null);
      ...
      React.useEffect(() => {
        if(contentId && contentId!==""){
                axios.get(`/api/v1/content/${contentId}`).then((response) => {
                    ...
                    <Grid item xs={7} >
                      <CardContent sx={{ flex: 1 , p: 2}} >    
                        <Grid container spacing={2} columns={{ xs: 4, md: 12 }} direction="column">   
                          ...
                          {versionSelected === null ? (
                          <><Button variant="contained" color="primary" sx={{ mr: 2 }} disabled={true}  type="button">
                            Текст засварлах
                            {/* <BookmarkIcon sx={{ fontSize: 20 }} /> */}
                          </Button><Button variant="contained" color="primary" disabled={true}>
                              Аудио засварлах
                              {/* <HeadphonesIcon sx={{ fontSize: 20 }} /> */}
                            </Button></>
                            ) : (
                            <><Button variant="contained" color="primary" sx={{ mr: 2 }} disabled={false} onClick={() => navigate(`/VersionText/${contentId}/${versionSelected}`)} type="button">
                              Текст засварлах
                              {/* <BookmarkIcon sx={{ fontSize: 20 }} /> */}
                            </Button><Button variant="contained" color="primary" disabled={false}  onClick={() => navigate(`/VersionAudio/${contentId}/${versionSelected}`)}>
                            Аудио засварлах
                                {/* <HeadphonesIcon sx={{ fontSize: 20 }} /> */}
                              </Button></>)}
                          </Grid>
