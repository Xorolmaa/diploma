...
export default function DeleteContent() {
  ...
  useEffect(() => {
    if(deleteAction){
            axios.delete(`/api/v1/delete-content/${contentId}`).then((response) => {
                console.log("deleted");
                    ...
                    <Grid item xs={7} >
                      <CardContent sx={{ flex: 1 , p: 2}} >    
                        <Grid container spacing={2} columns={{ xs: 4, md: 12 }} direction="column">   
                          ...
                          {/* {versionSelected === null ? (
                          <><Button variant="contained" color="error" sx={{ mr: 2 }} disabled={true} type="button">
                            Устгах
                          </Button></>
                            ) : (
                            <><Button variant="contained" color="error" sx={{ mr: 2 }} disabled={false} onClick={deleteVersion} type="button">
                              Устгах
                            </Button></>)} */}
